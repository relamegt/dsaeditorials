# Find the Nth Root of an M - Solution
...
1. Read `N` and `M`.
2. Iterate from `1` to `M`.
3. Compute `iⁿ`.
4. If the value equals `M`, print `i`.
5. If the value exceeds `M`, stop the search.
6. If no integer root exists, print `-1`.
...
1. If no exact root exists, print `-1`.
